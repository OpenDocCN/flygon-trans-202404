...
实际数据与用于训练[葡萄酒质量数据集](https://archive.ics.uci.edu/ml/datasets/wine+quality)的数据相同，该数据以CSV文件的形式存储在本地。文件被读入内存，然后记录从文本记录转换为protobuf记录，这些记录通过一个预定义的发送间隔的无限循环发布到Kafka主题。
