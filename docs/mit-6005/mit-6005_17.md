...
另一个需要观察的是，此语法允许端口号不符合技术上的规定，因为端口号只能范围从0到65535。我们可以编写更复杂的端口定义，只允许这些整数，但在语法中通常不这样做。相反，约束0 <= 端口 <= 65535 将与语法一起指定。
...
但是右侧的`html`的递归使用无法消除，并且也不能简单地用重复运算符替换。因此，HTML语法不是正则的。
