...
我们如何确保这些Tweet对象是不可变的——即，一旦创建了一条推文，其作者、消息和日期就永远不能更改？
...
`retweetLater`接受一个tweet，并应该返回另一个消息相同（称为*转推*）但晚一个小时发送的tweet。`retweetLater`方法可能是一个自动回声Twitter名人说的有趣事情的系统的一部分。
