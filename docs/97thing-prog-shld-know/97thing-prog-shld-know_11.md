...
你挠着头，想知道这段代码可能是做什么的。它似乎从一个交易员对象中获取一个ID，然后使用它从一个，嗯，显然是一个地图-地图的地图中获取一个映射，然后看看另一个来自投资组合对象的ID是否存在于内部地图中。你更加挠头了。你寻找portfolioIdsByTraderId的声明，发现了这个：
...
然后我们有了用户定义类型！好吧，这不是新闻，但它确实改变了游戏规则。如果你的领域包含像交易员和投资组合这样的概念，你可以使用名为Trader和Portfolio的类型来对它们建模。但更重要的是，你也可以使用领域术语来建模它们之间的*关系*。
