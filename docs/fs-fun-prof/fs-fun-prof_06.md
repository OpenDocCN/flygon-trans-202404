...
这是一个非常重要的主题，你必须了解偏函数的工作原理。请参阅系列文章["函数式思维"](thinking-functionally.html)以获取更详细的讨论。
...
记住，如果“else”分支丢失，就假设它返回unit，因此“true”分支也必须返回unit。
...
原因是“`Bind`”期望元组`（wrapper，func）`，而不是两个参数。（检查F#文档中bind的签名）。
