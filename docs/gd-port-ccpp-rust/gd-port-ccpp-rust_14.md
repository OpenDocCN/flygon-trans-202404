...
如果我们在实现中未定义`TOOLTIP_VERSION`为与调用方相同的值，则该代码可能会覆盖整个内存，因为它认为在一个地方结构体有128字节，而在另一个地方有64字节。
...
Rust还允许我们的结构实现另一个称为`Drop`的trait，它相当于C++的析构函数。
...
但如果我们等到异常抛出后才分配内存，那么`buffer_`可能就不会被设置为NULL，因此我们必须确保将其初始化为NULL。
