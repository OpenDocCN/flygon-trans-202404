...
[<seqC-case-take-2>](#(elem._(chunk._~3cseq.C-case-take-2~3e~3a1))) ::=
...
而其他语言会重载突变语法（=或:=），在Racket中它们保持不同：set!用于突变变量。这迫使Racket程序员面对我们在突变：结构和变量开头介绍的区别。当然，在我们的核心语言中，我们将通过使用不同的构造来避开这些语法问题，用于盒子和变量。
